...
} = require("../services/centryosCheckoutService");
...
function buildDepositRedirectUrl(depositId) {
...
function paymentLinkResponse(deposit) {
    ...
    return {
        ...
        paymentLinkId: deposit.providerPaymentLinkId,
...
exports.createMyPaymentLink = async (req, res) => {
    ...
    try {
        ...
        const itemDeliveryAddress = String(
            req.body?.itemDeliveryAddress || ""
        ).trim();

        if (!itemDeliveryAddress) {
            return res.status(400).json({
                success: false,
                message:
                    "itemDeliveryAddress is required by CentryOS. Send the customer's real billing or delivery address."
            });
        }

        const clientReference = req.body?.clientReference
            ? String(req.body.clientReference).trim()
            : null;
        ...
        deposit = await prisma.deposit.create({
            data: {
                ...
                amount,
                currency,
                method: "CENTRYOS_PAYMENT_LINK",
                provider: "CENTRYOS",
                clientReference,
                status: "PENDING",
                providerStatus: "CREATING_LINK"
            }
        });

        const result = await createCentryosPaymentLink({
        ...
        const savedDeposit = await prisma.deposit.update({
            where: {
                id: deposit.id
            },
            data: {
                paymentId: result.paymentLinkId,
                providerPaymentLinkId:
                    result.paymentLinkId,
                paymentUrl: result.paymentUrl,
                paymentToken: result.token,
                paymentTokenType: result.tokenType,
...
exports.getMyPaymentLinkDeposit = async (req, res) => {
